...
<p>To begin with our population will be randomly generated. This set of states will be generation 0, or 1 (or whatever you want). To get to our next generation we need to evaluate how well each state meets our goal. This means we need some sort of evaluation function that takes our state as input and assigns a numeric value to it. For the TSP example I just used the total distance of the path (in this case a lower score was better).</p>

<p>Once we have a score for the states in our generation we are ready to create the next generation. In my implementation I chose to go from two parents to one two children, some implementations go from two parents to one child. Either way, we chose the parents by randomly selecting two parents where each parent’s chance of being selected is weighted by their score that we assigned earlier with better performing individuals having a better chance of being chosen.</p>

<p>Once we have selected the two parents, we select a section of the string of “characters” (the length of which is determined by our crossover factor). Note: when I say characters I mean any element in an array, strings are just the default data type I have seen. The same area is selected in both strings and then we swap the “characters” between the two parents. </p>

<p>After performing the cross-over we are almost done. The last thing that is done in my implementation is to randomly mutate some part of the string, this could be no characters or all of them. The mutation rate is another parameter that we tune by hand (at least in this case, there might be more advanced techniques to dynamically set this value). </p>

<p>That whole process is to create two children, we then just repeat the process as many times as we need to create the next generation and then repeat until we are satisfied with the result.
          </p>
        </div>
      </div>
    </section>
